...
target_langs<-c("French (French)", "French (Quebecois)", "Spanish (Mexican)", "Spanish (European)",  "German", "Portuguese (European)", "Hungarian", "Swedish", "Italian", "Norwegian","Turkish", "Dutch")

target_langs<-c("Portuguese (European)","Hungarian","Spanish (Mexican)", "Spanish (European)",  "German") # vector memory exhausted (limit reached in line 210)
target_langs<-c("French (French)","French (Quebecois)")
target_langs<-c("Italian","Norwegian", "Turkish") 
target_langs<-c("Swedish") #Danish Dutch problem with imputation

#target_langs <- c("Dutch", "Portuguese (European)" )

target_langs <- unique(target_langs) 
...
#get_token_metrics("German", metric_funs, corpus_args)
#get_uni_lemma_metrics("German", build_uni_lemma_map(uni_lemmas|> filter(language =="German")))

```

Loading cached CHILDES data for multiple languages:
```{r load_childes_xling, eval=TRUE}
...
predictor_data_lexcat <- map_df(target_langs, prep_lexcat,predictor_data, uni_lemmas, ref_cat) |>
  distinct()
...
predictor_data_lexcat  <- predictor_data_lexcat  |>distinct()
```
...
predictor_data_imputed_scaled <- map_df(target_langs, do_full_imputation, predictor_data_lexcat |> filter(language %in% target_langs), predictor_sources, max_steps)
...
#i<- do_full_imputation(target_langs3,predictor_data_lexcat |>filter(language %in% target_langs3), predictor_sources, max_steps)

#predictor_data_imputed_scaled <- bind_rows(predictor_data_imputed_scaled1, predictor_data_imputed_scaled2)
...
run_main_model<-function(lang, aoa_predictor_data, predictors, uni_lemmas, ref_cat, lexcat_interactions = TRUE){
  ...
  aoa_predictor_data_lexcat <- prep_lexcat(lang, aoa_predictor_data, uni_lemmas |> filter(language==lang), ref_cat) ## Set again lexical category contrasts
...
run_cross_validation_model<-function(lang, meas, aoa_predictor_data, predictors, uni_lemmas, ref_cat){
  ...
  file__ <-  glue("{childes_path}/xval_{l}_{meas}.rds")
  if (file.exists(file__)) {
    loo_df <- readRDS(file__)
  }else{
  aoa_predictor_data <- aoa_predictor_data|> select(-lexical_category)|>
